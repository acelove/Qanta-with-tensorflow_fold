...
def train(rnn, saver, node_dict,tree_dict):
    ...
    for _ in xrange(FLAGS.num_epochs):
      loss_sum = 0  
      random.shuffle(node_dict['train'])
      shuffle_neg(tree_dict['train'])
    ...
    for tt, split in enumerate([train_q, test_q]):
      ...
      for qid in split:
        ...
        if tt == 0:
            ...
            print "\rProcessing train features ...... %d / %d" % (count, len(split)),
            sys.stdout.flush()
        else:
            test_feats.append( (curr_feats, tree.ans.lower()) )
            print "\rProcessing test features ...... %d / %d" % (count, len(split)),
            sys.stdout.flush()
        count += 1
